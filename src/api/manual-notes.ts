...
async function handleIngest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    let extraction = null;
    try {
      console.log(`[ManualNote] Auto-extracting: ${sourceMaterial.title}`);
      const extractionResult = await extractFromContent(body.content, 'manual_note');
      ...
      if (extractionError) {
        console.error('[ManualNote] Extraction save failed:', extractionError);
      } else {
        extraction = extractionData;
        console.log(`[ManualNote] Extraction created: ${extraction.id}`);
      }
    } catch (extractErr) {
      console.error('[ManualNote] Auto-extraction failed:', extractErr);
      // Don't fail the request - source material was saved successfully
    }
